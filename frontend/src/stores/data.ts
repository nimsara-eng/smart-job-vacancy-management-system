...
export const useDataStore = defineStore('data', {
  ...
  actions: {
    // ── Persistence Helper ──────────────────────────────────────────────────
    // Saves current state to the browser's local storage
    saveData() {
    ...
    async loadAll() {
      ...
      this.error = null
      try {
        // ALWAYS fetch the freshest JSON files generated by the C++ backend
        // We removed the Local Storage check so the C++ data is the single source of truth
        ...
        // Save these fresh files to Local Storage for current session use
        this.saveData()
